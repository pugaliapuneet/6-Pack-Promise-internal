...
export const updateFavoriteWorkouts = (id = null) => async (dispatch, getState) => {
    const { user: { favoriteIds } } = getState()
    ...
    if (id !== null) {
        payload = payload.includes(id) ? payload.filter(i => id != i) : [...payload, id]
    }
    await AsyncStorage.setItem(localKeys.favoriteWorkouts, JSON.stringify(payload))
    dispatch({ type: ActionTypes.UPDATE_FAVORITE_WORKOUTS, payload })
}

export const updateSelfies = (data, remove = false) => async (dispatch, getState) => {
    const { user: { selfies } } = getState()
    let payload = remove ? selfies.filter(i => i.uri != data.uri) : [...selfies, data]
...
export const getPurchasedStatus = () => async (dispatch) => {
    ...
    dispatch({ type: ActionTypes.UPDATE_PURCHASED_FULL_VERSION, payload: Boolean(payload) })
}
